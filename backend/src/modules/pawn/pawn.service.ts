import { Prisma, PawnTicket, PawnItem, Customer } from "@prisma/client";
...
type TicketWithDetails = PawnTicket & {
  items: PawnItem[];
  customer: Customer | null;
};

export class PawnService {
  private formatTicket(t: TicketWithDetails) {
    return {
      id: t.id,
      ...
      status: t.status,
      settled_date: t.settledDate?.toISOString() ?? null,
      items: t.items.map((i) => ({
        id: i.id,
        name: i.name,
        type: i.type,
